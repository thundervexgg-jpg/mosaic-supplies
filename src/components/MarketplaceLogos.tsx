/**
 * Marketplace marks for the "where we sell" band.
 *
 * eBay is their official single-colour mark. Amazon is a clean lockup of the wordmark
 * and the smile rather than a traced copy of the registered logo.
 *
 * Before this page goes out on trade applications, swap both for the official badges —
 * Amazon supplies an "Available at Amazon" asset pack to sellers through Seller Central,
 * and that licensed badge is what a supplier-facing site should carry. Drop the files in
 * /public/marketplaces/ and replace the two components below; nothing else changes.
 */

type Props = { className?: string }

export function EbayLogo({ className }: Props) {
  return (
    <svg role="img" viewBox="0 0 24 24" className={className} fill="currentColor" aria-label="eBay">
      <title>eBay</title>
...
export function AmazonLogo({ className }: Props) {
  return (
    <span className={`inline-flex flex-col items-center leading-none ${className ?? ''}`} aria-label="Amazon">
      <span
        className="lowercase"
        style={{
      ...
      {/* The smile, running a to z. */}
      <svg viewBox="0 0 100 18" className="mt-[0.12em] w-full" fill="none" aria-hidden="true" focusable="false">
        <path
          d="M2 4C18 14.5 40 19 58 19c12 0 23-2.4 32-7"
          stroke="currentColor"
          strokeWidth="5"
          strokeLinecap="round"
        />
        <path d="M84 3.5 97 9.5 86 17Z" fill="currentColor" />
      </svg>
    </span>
  )
}
